...
def find_first_available_today(start) -> [Doctor, datetime]:
    ...
    doctors = db.session.scalars(db.select(Doctor).join(Schedule).filter(Schedule.start_time <= start.time(), Schedule.end_time > start.time(), Schedule.day_of_week == DayOfWeek(start.weekday()))).all()
    ...
    appointments = db.session.scalars(db.select(Appointment).filter(Appointment.doctor_id.in_([doctor.id for doctor in doctors]), Appointment.end_time > start, Appointment.start_time <= start)).all()

    # Find doctors without appointments
    available_doctors = [doctor for doctor in doctors if len([appointment for appointment in appointments if appointment.doctor_id == doctor.id]) == 0]
